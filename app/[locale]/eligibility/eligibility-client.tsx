...
export default function EligibilityPageClient() {
                ...
                <div className="mt-8">
                  ...
                  <ul className="mt-4 space-y-4">
                    {eligibleResults.map((item) => (
                      <EligibleBenefitCard key={item.id} id={item.id} t={t} />
                    ))}
                  </ul>
                </div>
              ) : (
                <p className="mt-8 rounded-lg bg-cream-muted px-4 py-4 text-sm text-ink-muted">
                  {t.results.noneEligible}
                </p>
              )}
                ...
                <div className="mt-10">
                  ...
                  <ul className="mt-4 space-y-3">
                    {ineligibleResults.map((item) => (
                      <IneligibleBenefitCard key={item.id} result={item} t={t} />
                    ))}
                  </ul>
                </div>
              )}
...
function EligibleBenefitCard({id, t}: {id: BenefitId; t: EligibilityMessages}) {
...
function IneligibleBenefitCard({
  result,
  t,
}: {
  result: BenefitResult;
  t: EligibilityMessages;
}) {
  ...
  return (
    <li className="rounded-lg border border-stone-200 bg-stone-50 p-4 opacity-90">
